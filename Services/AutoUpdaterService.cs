    ...
    public static async Task CheckForUpdatesAsync(Action<string> onProgress)
    {
        try
        {
            ...
            if (release != null && release.Id != SettingsService.Instance.LastUpdateId)
            {
                ...
                {
                    if (asset.Name.Equals("CIDE_Setup.exe", StringComparison.OrdinalIgnoreCase) && OperatingSystem.IsWindows())
                    {
                ...
                if (!string.IsNullOrEmpty(downloadUrl))
                {
                    ...
                    var tempUpdatePath = Path.Combine(AppPaths.UpdatesDir, "update.tmp");
                    using (var fs = new FileStream(tempUpdatePath, FileMode.Create, FileAccess.Write, FileShare.None))
                    {
                    ...
                    File.Move(tempUpdatePath, currentExe, overwrite: true);
